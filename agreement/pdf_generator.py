...
def _find_font(name):
    """Find font file cross-platform (Windows dev + Linux production)."""
    candidates = []
    ...
    candidates += [
        ...
        f'/usr/share/fonts/truetype/liberation/{name.replace("arial", "LiberationSans").replace("arialbd", "LiberationSans-Bold")}',
...
try:
    _arial_path = _find_font('arial.ttf')
    _arialbd_path = _find_font('arialbd.ttf')
    if _arial_path and _arialbd_path:
        pdfmetrics.registerFont(TTFont('Arial',      _arial_path))
        pdfmetrics.registerFont(TTFont('Arial-Bold', _arialbd_path))
        _F  = 'Arial'
        _FB = 'Arial-Bold'
    else:
        raise FileNotFoundError("Arial not found")
except Exception:
    _F  = 'Helvetica'
    _FB = 'Helvetica-Bold'

RS    = '\u20b9'
GREEN = colors.HexColor('#6366F1')
...
def S(name):
    return {
        ...
        'ref':      ParagraphStyle('ref',      fontName=_FB, fontSize=8,  textColor=GREEN, alignment=TA_CENTER, spaceAfter=5),
        'section':  ParagraphStyle('section',  fontName=_FB, fontSize=9,  textColor=GREEN, spaceBefore=3, spaceAfter=2, leading=12),
...
def generate_agreement_pdf(agreement, base_url: str = 'https://weblancehub.in') -> bytes:
    buf = io.BytesIO()
    ...
    # ── HEADER ────────────────────────────────────────────────────────
    hdr = Table([[
        Paragraph('<font color="#6366F1">WEB</font>LANCE', S('brand')),
        Paragraph(
            'Devanahalli, Karnataka, India<br/>'
            '+91 7892934437 | infoweblance01@gmail.com | weblancehub.in',
            S('contact')
        ),
    ]], colWidths=[W*0.4, W*0.6])
    hdr.setStyle(_ts([('VALIGN',(0,0),(-1,-1),'MIDDLE'),('BOTTOMPADDING',(0,0),(-1,-1),4)]))
    story.append(hdr)
    story.append(HRFlowable(width='100%', thickness=2, color=GREEN, spaceAfter=4))
    ...
    story.append(Paragraph('3. PROJECT TIMELINE', S('section')))
    story.append(tl_tbl)
    story.append(Spacer(1, 3))
    ...
    story.append(Paragraph('4. PAYMENT TERMS', S('section')))
    ...
    if agreement.client_signed and agreement.client_signed_at:
        c_date = f'Signed on {agreement.client_signed_at.strftime("%d %b %Y")}'
        c_signed_mark = '✓ Digitally Signed'
    else:
        c_date = '___________'
        c_signed_mark = ''
    ...
    sig_tbl = Table([
        [Paragraph('<b>For Weblance (Service Provider)</b>', S('cell_b')),
         Paragraph('<b>Client Signature</b>', S('cell_b'))],
        [
            Paragraph(f'<br/><br/><br/>__________________________<br/><b>Balakrishna</b><br/>Founder &amp; Lead Developer, Weblance<br/>Date: {w_date}', S('small')),
            Paragraph(f'<br/><br/><br/>__________________________<br/><b>{agreement.client_name}</b><br/>{c_co + "<br/>" if c_co != "—" else ""}{c_signed_mark}<br/>Date: {c_date}', S('small')),
        ],
    ...
    final = [
        ...
        Paragraph(
            ...
            f'Generated: {date.today().strftime("%d %B %Y")} &nbsp;|&nbsp; weblancehub.in',
            S('footer')
        ),
    ]
...
def S(name):
    return {
        ...
        'ref': ParagraphStyle('ref', fontName=_FB, fontSize=8.5, textColor=GREEN, alignment=TA_CENTER, spaceAfter=6),
        'section': ParagraphStyle('section', fontName=_FB, fontSize=10, textColor=GREEN, spaceBefore=4, spaceAfter=3, leading=14),
...
def generate_agreement_pdf(agreement, base_url: str = 'https://weblancehub.in') -> bytes:
    ...
    story.append(hdr)
    story.append(HRFlowable(width='100%', thickness=2, color=GREEN, spaceAfter=5))
    ...
    story.append(KeepTogether([Paragraph('4. PROJECT TIMELINE', S('section')), tl_tbl, Spacer(1, 3)]))
    ...
    # Signature table
    weblance_date = 'Signed' if agreement.weblance_signed else '___________________________'
    if agreement.client_signed and agreement.client_signed_at:
        client_date = f'Signed on {agreement.client_signed_at.strftime("%d %b %Y")}'
        client_signed_mark = '<font color="#6366F1"><b>✓ Digitally Signed</b></font>'
    else:
        client_date = '___________________________'
        client_signed_mark = ''
    client_co = _clean(agreement.company_name)

    sig_tbl = Table([
        [Paragraph('<b>For Weblance (Service Provider)</b>', S('cell_bold')),
         Paragraph('<b>Client Signature</b>', S('cell_bold'))],
        [
            Paragraph(
                '<br/><br/><br/>'
                '___________________________<br/>'
                f'<b>Balakrishna</b><br/>Founder &amp; Lead Developer, Weblance<br/>Date: {weblance_date}',
                S('small')
            ),
            Paragraph(
                '<br/><br/><br/>'
                ...
                f'{client_signed_mark}<br/>Date: {client_date}',
                S('small')
            ),
        ],
    ], colWidths=[W * 0.5, W * 0.5])
